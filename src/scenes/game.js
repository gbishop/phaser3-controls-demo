...
export class Game extends Scene {
  ...
  create() {
    ...
    this.scrollingBg = this.add.tileSprite(
      this.norm_X,
      this.norm_Y,
      396,
      529,
    ...
    this.bottomSpikes = this.add.sprite(
      this.norm_X,
      this.norm_Y + 240,
      "spike"
    );
    ...
    // do these once instead of in update

    // overlap between rocket and spikes
    this.physics.add.overlap(
      this.rocket,
      this.topSpikes,
      this.rocketCollidedWithSpike,
      null,
      this
    );
  ...
  resize() {
    ...
    cam.centerToBounds();
    cam.setScroll(
      -window.innerWidth / 2 + this.norm_X,
      -window.innerHeight / 2 + this.norm_Y
    );
  ...
  rocketCollideWithALien(rocket, alien) {
    if (!this.canUpdateAlien) {
      //Overlap runs multiple frames, we only want it to run once
      return;
    }
